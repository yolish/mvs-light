...
from models.nets import MVSLight
from utils import *
...
parser.add_argument('--num_view', type=int, default=5, help='threshold of num view')
#filter 
...
def save_scene_depth(testlist):
    ...
    test_dataset = MVSDataset(args.testpath, testlist, "test", args.num_view, args.numdepth, args.interval_scale,
                              max_h=args.max_h, max_w=args.max_w, fix_res=args.fix_res)
    test_loader = DataLoader(test_dataset, args.batch_size, shuffle=False, num_workers=4, drop_last=False)
    ...
    if args.loadckpt is None:
        print("No checkpint provided - testing an untrained model")
    else:
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(test_loader):
            ...
            cams = sample["proj_matrices"][0].numpy()
            imgs = sample["imgs"].numpy()
            print('Iter {}/{}, Time:{} Res:{}'.format(batch_idx, len(test_loader), end_time - start_time, imgs[0].shape))

            # save depth maps and confidence maps
            for filename, cam, img, depth_est in zip(filenames, cams, imgs, \
...
def filter_depth(pair_folder, scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        final_mask = geo_mask_sum >= args.thres_view

        os.makedirs(os.path.join(out_folder, "mask"), exist_ok=True)
        save_mask(os.path.join(out_folder, "mask/{:0>8}_final.png".format(ref_view)), final_mask)

        print("processing {}, ref-view{:0>2}, final-mask:{}".format(scan_folder, ref_view,
                                                                                    final_mask.mean()))
        ...
        if args.display:
            ...
            cv2.imshow('ref_depth * geo_mask', ref_depth_est * final_mask.astype(np.float32) / 800)
            cv2.waitKey(0)
